        ...
        onSubmit={async (values, { setSubmitting, resetForm }) => {
          ...
          try {
            ...
            if (values.file) {
              ...
              const filteredPhrases = values.phraseList.filter(
                (p) => p.trim() !== "",
              );
            ...
            setResult(
              `An error occurred during transcription: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
          } finally {
            setLoading(false);
            setSubmitting(false);
        ...
        {({ handleChange, values, setFieldValue }) => {
          const handleLanguageDetect = async () => {
            ...
            if (values.file.size > 50 * 1024 * 1024) {
              alert(
                "File is too large for auto-detection (max 50MB). Please select language manually.",
              );
              return;
            }
            ...
            try {
              console.log(
                `Detecting language using: ${LOGIC_API_URL}/detect-language`,
              );
              ...
              if (!res.ok) {
                ...
                throw new Error(
                  `Failed to detect language: ${res.status} ${res.statusText} - ${errText}`,
                );
              }
              ...
              if (text.trim().startsWith("{")) {
                try {
                  ...
                  if (json.language)
                    detectedLanguage = String(json.language).toUpperCase();
                  else if (json.detected_language)
              ...
              if (detectedLanguage === "POLISH" || detectedLanguage === "PL") {
                ...
                if (detectedLanguage && detectedLanguage.length > 0) {
                  ...
                  alert(
                    "Could not detect language. The audio might be unclear or the format unsupported.",
                  );
                }
              }
            } catch (error) {
              ...
              alert(
                `An error occurred during language detection: ${error instanceof Error ? error.message : "Unknown error"}`,
              );
            } finally {
              setLoading(false);
            }
                        ...
                        {values.phraseList.map((phrase, index) => (
                          <div key={index} className="flex gap-2">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const newList = values.phraseList.filter(
                                  (_, i) => i !== index,
                                );
                                setFieldValue("phraseList", newList);
                              }}
