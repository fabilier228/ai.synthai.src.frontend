  ...
  const mainSteps = [
    {
      ...
      description:
        "User selects a category: song, conversation, lecture, or audiobook",
      color: "primary",
      bgColor: "primary_muted",
    },
    {
      ...
      description:
        "The system automatically transcribes audio and detects the language",
      color: "secondary",
      bgColor: "secondary",
    },
    {
      ...
      <div className="max-w-7xl mx-auto">
                ...
                <div key={idx}>
                  <div
                    className={`relative bg-surface rounded-2xl p-4 md:p-6 shadow border-2 transition-all duration-300 ${
                      isActive
                        ? "border-primary md:shadow-2xl md:scale-105"
                        : "border-outline"
                    }`}
                  >
                    <div className="flex flex-col sm:flex-row items-start gap-3 md:gap-6">
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{ backgroundColor: resolveColor(step.color) }}
                        >
                          {step.number}
                        </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-h4 md:text-h3 font-heading text-text mb-1 truncate">
                          {step.title}
                        </h3>
                        <p className="text-muted text-body_sm md:text-body_md leading-snug line-clamp-3">
                          {step.description}
                        </p>
                      </div>
                    ...
                    {isActive && (
                      <div className="mt-3 md:mt-4 h-1 md:h-2 bg-outline rounded-full overflow-hidden">
                        <div
                          className="h-full animate-[progress_8s_linear]"
                          style={{
                            width: "100%",
                            backgroundColor: resolveColor(step.color),
                          }}
                        />
                      </div>
                    )}
            ...
            {audioTypes.map((type, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-h5 md:text-h4 font-heading text-text text-center mb-4">
                    {type.title}
                  </h3>
                  <div className="space-y-2">
                    {type.results.map((result, rIdx) => (
                      <div
                        key={rIdx}
                        className="flex items-start gap-2 text-body_md text-muted"
                      >
                        <Check
                          className="w-4 h-4 flex-shrink-0 mt-0.5"
                          style={{ color: resolveColor(type.color) }}
                        />
                        <span>{result}</span>
                      </div>
                    ))}
            ...
            {[
              {
                icon: Shield,
                title: "Azure AI",
                desc: "Multilingual transcription",
                color: "primary",
              },
              {
                icon: Sparkles,
              ...
              return (
                ...
                >
                  <TechIcon
                    ...
                    style={{ color: resolveColor(tech.color) }}
                  />
                  <h4 className="font-heading text-h5 mb-2 text-text">
                    {tech.title}
                  </h4>
        ...
        <div className="text-center px-4">
          <div className="bg-gradient-to-r from-primary via-tertiary to-secondary rounded-3xl p-6 md:p-12 text-white max-w-4xl mx-auto shadow-2xl">
            <h2 className="text-h3  md:text-h2 font-heading mb-4">
              Ready to try?
            </h2>
